...
def home():
    ...
    # Check karo ki URL mein agent ka referral code hai ya nahi
    agent_ref = request.args.get('ref')
    if agent_ref:
        conn = sqlite3.connect(DATABASE); c = conn.cursor()
        # Check karo ki yeh user ek valid agent hai
        c.execute("SELECT user_id FROM agents JOIN users ON agents.user_id = users.id WHERE users.username = ?", (agent_ref,))
        agent = c.fetchone()
        conn.close()
        if agent:
            # Agent ko session mein save karo
...
def register_page():
    ...
    if request.method == 'POST':
        ...
        referrer_username_from_form = request.form.get('referrer')
        device_fingerprint = request.form.get('device_fingerprint', 'unknown')
        if password != confirm_password:
            flash("Passwords match nahi ho rahe!", "error")
            return redirect(url_for('register_page', ref=referrer_username_from_form))
        password_hash = generate_password_hash(password)
        conn = sqlite3.connect(DATABASE)
        c = conn.cursor()
        try:
            c.execute("INSERT INTO users (username, password_hash, device_fingerprint) VALUES (?, ?, ?)", 
                      (username, password_hash, device_fingerprint))
            new_user_id = c.lastrowid
            conn.commit()
            if referrer_username_from_form:
                c.execute("SELECT id FROM users WHERE username = ?", (referrer_username_from_form,))
                referrer_user = c.fetchone()
                if referrer_user:
                    referrer_id = referrer_user[0]
                    c.execute("SELECT COUNT(id) FROM users WHERE device_fingerprint = ?", (device_fingerprint,))
                    device_count = c.fetchone()[0]
                    if device_count <= 1 and referrer_id != new_user_id:
                        ...
                        reward = PRICE_CONFIG.get("refer_reward", 2)
                        c.execute("UPDATE users SET wallet = wallet + ? WHERE id = ?", (reward, referrer_id))
                        conn.commit()
                    elif device_count > 1:
                        flash("Account ban gaya, lekin is device se pehle bhi register ho chuka hai, isliye referral bonus nahi mila.", "warning")
            if not 'warning' in [msg[0] for msg in get_flashed_messages(with_categories=True)]:
            ...
            if 'UNIQUE constraint failed: users.username' in str(e):
                ...
                flash(f"Ek database error aa gaya: {e}", "error")
            return redirect(url_for('register_page', ref=referrer_username_from_form))
        finally:
            conn.close()
    return render_template('register.html', referrer_username=referrer_username)
...
# === "CHECKOUT" ROUTE (BUG FIX KE SAATH UPDATE HUA) ===
@app.route('/checkout/<int:product_id>')
def checkout_page(product_id):
    if 'user_id' not in session:
        flash("Khareedne ke liye please login karein.", "error")
    ...
    if product is None:
        flash("Yeh product available nahi hai.", "error")
        return redirect(url_for('home'))
...
def pay_page(product_id):
    if 'user_id' not in session:
        return redirect(url_for('login_page'))
        
    user_id = session['user_id']
    
    # Check karo ki kya user kisi agent ke link se aaya tha
    agent_id = session.pop('referrer_agent_id', None) # Session se nikalo aur clear karo
    ...
    if product is None:
        flash("Product nahi mila.", "error"); conn.close(); return redirect(url_for('home'))

    # === NAYA LOGIC: Order save karte waqt agent_id bhi save karo ===
    ...
    flash(f"Aapka order ({product['name']}) receive ho gaya hai! Admin jald hi approve karenge.", "success")
